# https://github.com/modichirag/GSM-VI/blob/main/gsmvi/advi.py, commit 1aa37f73773418fa4a8279ef9cbe151f2ad58eb0
import numpy as np
import jax
...
class ADVI:
    ...
    def __init__(self, D, lp, stl_estimator=True):
        """
        Inputs:
        ...
        self.stl_estimator = stl_estimator
    ...
    def neg_elbo(self, params, key, batch_size):
        ...
        scale_tril = jnp.zeros((self.D, self.D))
        scale_tril = scale_tril.at[self.idx_tril[:, 0], self.idx_tril[:, 1]].set(scales)
        samples = MultivariateNormal(loc=loc, scale_tril=scale_tril).sample(
            key, (batch_size,)
        )

        if self.stl_estimator:
            loc = jax.lax.stop_gradient(loc)
            scale_tril = jax.lax.stop_gradient(scale_tril)
        logl = jnp.sum(self.lp(samples))
        logq = jnp.sum(
            MultivariateNormal(loc=loc, scale_tril=scale_tril).log_prob(samples)
        )
        elbo = logl - logq
        negelbo = -1.0 * elbo
        return negelbo
    ...
    def fit(
        ...
        def opt_step(params, opt_state, key):
            loss, grads = jax.value_and_grad(lossf, argnums=0)(
                params, key, batch_size=batch_size
            )
        ...
        for i in range(niter + 1):
            ...
            if monitor is not None:
                if (i % monitor.checkpoint) == 0:
                    mean = params[0]
                    cov = self.scales_to_cov(params[1] * 1.0)
    ...
    def fit_with_history(
        ...
        nevals = 1
        means = [np.array(mean)]
        covs = [np.array(self.scales_to_cov(scales * 1.0))]
